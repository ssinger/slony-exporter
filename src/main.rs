...
use tokio::prelude::*;
use slony::{SlonyStatus,SlonyConfirm};
use metrics::{Metrics};
...
async fn metric(r:Request<Body>)->Result<Response<Body>,Infallible> {
    ...
    match r {
        ...
        Err(x) =>  Ok(Response::builder()
                      .status(500)
                      .body(Body::from(String::from(x.message()))).unwrap())
    }
...
fn metric_for_connection(slony_connection : &SlonyStatus,metrics : &Metrics) {
    ...
    metrics.last_event().with_label_values(&[&format!("{}",slony_connection.node_id())])
        .set(slony_connection.last_event());
    metrics.last_event_timestamp().with_label_values(&[&format!("{}",slony_connection.node_id())])
