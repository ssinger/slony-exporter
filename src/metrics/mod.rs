use prometheus::{Opts,Registry,Counter,TextEncoder,Encoder,IntGaugeVec};
...
impl Metrics {
    ...
    pub fn new()->Metrics {
        ...
        let last_confirmed_event = register_int_gauge_vec!(opts,
                                                           &["slony_origin","slony_receiver"]).unwrap();

        let opts = Opts::new("slony_confirmed_event_timestamp",
                             "The timestamp of the last event replicated and confirmed");
        let last_confirmed_event_timestamp = register_int_gauge_vec!(opts,
                                                                     &["slony_origin","slony_receiver"]).unwrap();
        
        
        let opts = Opts::new("slony_last_event","The last event generated");
        let last_event = register_int_gauge_vec!(opts,
                                                 &["slony_origin"]).unwrap();

        let opts = Opts::new("slony_last_event_timestamp","The timestamp of the last generated event");
        let last_event_timestamp = register_int_gauge_vec!(opts,
                                                           &["slony_origin"]).unwrap();

        let opts = Opts::new("slony_received_event","The last event received from a remote node");
        let last_received_event = register_int_gauge_vec!(opts,
                                                           &["slony_origin","slony_receiver"]).unwrap();

        let opts = Opts::new("slony_received_event_timestamp",
                             "The timestamp of the last event replicated to this node from a remote node");
        let last_received_event_timestamp = register_int_gauge_vec!(opts,
                                                                    &["slony_origin","slony_receiver"]).unwrap();
        
        
        Metrics { last_confirmed_event: last_confirmed_event,
