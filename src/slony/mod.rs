...
impl SlonyStatus {
    ...
    pub fn new(node_id: i32, last_event_id: i64,
               last_event_timestamp: i64)->SlonyStatus {
        SlonyStatus { node_id: node_id, last_event_id: last_event_id,
...
pub fn fetch_slony_status() -> Result<SlonyStatus,Error> {
    ...
    let url = match url_opt {
        Ok(s)=>s,
        Err(_e)=>return Err(Error { message: String::from("Must set POSTGRES_URL")})
    };
...
fn query(url : &str)->Result<SlonyStatus,Error> {
    ...
    let slony_schema = match slony_schema_opt {
        Ok(s)=> s,
        Err(_e)=> return Err(Error{ message: String::from("SLONY_CLUSTER must be set")})
    };
...
fn fetch_node_data(client: &mut Client,slony_schema: &str)->Result<SlonyStatus,Error> {
    ...
    let query = format!(" select ev_origin, \
                          ev_seqno, \
    ...
    for row in event_rows {
        return Ok(SlonyStatus::new(  row.get(0),
                                     row.get(1),
                                     row.get(2)))
    }//for
    
    return Err(Error { message: String::from("No events found")})
}

fn fetch_node_confirmations(client: &mut Client, slony_schema: &str,
                            mut slony_status:  SlonyStatus)->Result<SlonyStatus,Error> {
...
fn fetch_node_incoming(client: &mut Client, slony_schema: &str,
                            mut slony_status:  SlonyStatus)->Result<SlonyStatus,Error> {
    let query = format!(" select ev_origin, \
                          ev_seqno, \
                          extract(epoch from ev_timestamp)::int8 \
...
#[cfg(test)]
mod tests {
    use crate::slony::{SlonyStatus, SlonyConnection};
